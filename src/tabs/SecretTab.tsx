...
setUserRooms(freshRooms)
  ...
  setEnteringRoom((current) =>
    ...
    className="w-full rounded-2xl border border-accent/50 bg-accent/15 px-4 py-4 text-sm font-black uppercase tracking-wider text-accent transition active:scale-[0.98] disabled:cursor-not-allowed disabled:border-white/10 disabled:bg-white/5 disabled:text-white/25 disabled:opacity-100"
  >
  {roomAttemptLoading
  ? 'ПРОВЕРКА...'
  : '🗝️ ПОПЫТАТЬСЯ ВОЙТИ'}
...
>
  ...
  КОМНАТ
    ...
    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-accent/10">
      ...
      {!questUnlocked ? (
  !mainDoorOpen ? (
    <div className="relative mt-0 flex min-h-[1100px] flex-col items-center">
      <button
  type="button"
  onClick={() => {
    setAdminPassword('')
...
>
  ...
  className="group relative z-20 mt-[60px] flex h-[180px] w-[128px] items-center justify-center overflow-hidden rounded-t-[120px] rounded-b-[28px] border-2 border-accent/80 bg-black/95 transition-all duration-300 active:scale-[0.97]"
  style={{
    boxShadow:
...
>
  ...
  <div
    className="absolute left-1/2 top-20 h-20 w-20 -translate-x-1/2 rounded-full border border-accent/20"
    style={{
      boxShadow:
...
const doorTheme =
 ...
 slotNumber === 1
  ? 'left-[55px] top-[330px]'
  : slotNumber === 2
    ? 'right-[55px] top-[330px]'
    : slotNumber === 3
      ? 'left-[55px] top-[480px]'
      : 'right-[55px] top-[480px]'
  }`}
  style={{
