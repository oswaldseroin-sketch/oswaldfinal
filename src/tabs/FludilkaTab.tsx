...
function renderMessage(
  ...
  className={`
   ...
   ${message.trim() === `[emoji:${emoji.id}]` ? 'animate-emojiPop' : ''}
    ${
...
<div
  ref={scrollRef}
 className="h-[420px] space-y-0.5 overflow-y-auto rounded-2xl bg-black/20 px-1 py-2"
    ...
    {selectedEmojis.map((emoji) => (
      ...
      {myNick && (
        <>
          ...
          <div className="relative mt-3 flex items-center gap-2 rounded-2xl border border-neon/30 bg-black/70 p-2 backdrop-blur-xl shadow-[0_0_25px_rgba(0,229,255,0.15)]">
            <button
...
{emojiOpen && (
  <div className="absolute bottom-full left-1/2 z-50 mb-3 w-[90%] max-w-md -translate-x-1/2 rounded-2xl border border-neon/40 bg-black/90 p-4 backdrop-blur-md"
    style={{
