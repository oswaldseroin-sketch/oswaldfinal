            ...
            {addType === 'person' && (
              <div className="mt-4">
                <div className="space-y-2.5">
                  ...
                  <div>
                    <input value={pOrg} onChange={(e) => setPOrg(e.target.value)} placeholder="Организация" className="h-14 w-full rounded-xl border border-line bg-input px-4 text-base text-ink outline-none focus:border-neon/50 placeholder:text-ink-faint" />
                    {pOrg && <button onClick={() => setPOrg('')} className="mt-0.5 text-[10px] font-bold text-ink-muted active:scale-95">× Очистить</button>}
                  </div>
                  <div>
                   <input value={pDate} onChange={(e) => setPDate(e.target.value)} placeholder="ДД.ММ.ГГГГ" className="h-14 w-full rounded-xl border border-line bg-input px-4 text-base text-ink outline-none focus:border-neon/50 placeholder:text-ink-faint" />
    ...
    ✓ УСПЕШНО ДОБАВЛЕН
            ...
            {addType === 'vehicle' && (
              <div className="mt-4">
                <div className="space-y-2.5">
                  <div>
                    <input value={vPlate} onChange={(e) => setVPlate(e.target.value)} placeholder="Госномер / название" className="h-14 w-full rounded-xl border border-line bg-input px-4 text-base text-ink outline-none focus:border-neon/50 placeholder:text-ink-faint" />
                    {vPlate && <button onClick={() => setVPlate('')} className="mt-0.5 text-[10px] font-bold text-ink-muted active:scale-95">× Очистить</button>}
                  </div>
                  <div>
                    <input value={vOrg} onChange={(e) => setVOrg(e.target.value)} placeholder="Организация" className="h-14 w-full rounded-xl border border-line bg-input px-4 text-base text-ink outline-none focus:border-neon/50 placeholder:text-ink-faint" />
                    {vOrg && <button onClick={() => setVOrg('')} className="mt-0.5 text-[10px] font-bold text-ink-muted active:scale-95">× Очистить</button>}
                  </div>
                  <div>
                   <input value={vType} onChange={(e) => setVType(e.target.value)} placeholder="Вид транспорта" className="h-14 w-full rounded-xl border border-line bg-input px-4 text-base text-ink outline-none focus:border-neon/50 placeholder:text-ink-faint" />
...
)}               <button onClick={() => void submitVehicle()} disabled={!vPlate.trim() || !vDate.trim()} className="mt-5 h-14 w-full rounded-xl bg-neon text-base font-extrabold text-bg transition-transform active:scale-95 disabled:opacity-40">ДОБАВИТЬ</button>
                <button onClick={() => setPanelMode('delete')} className="mt-2 flex h-11 w-full items-center justify-center gap-2 rounded-lg border border-error/40 bg-error/10 text-sm font-extrabold text-error transition-transform active:scale-95">
